# ---- III. Predictive Modelling: Version 3.5 ----
#
# Nested Cross-Validation (glmnet-only):
#   inner  - MANUAL K-fold CV with glmnet (+ OOF reconstruction), grouped-style CVM/SE
#            -> select winners for lambda.min and lambda.1se across α-grid
#   outer  - glmnet fit on OUTER-TRAIN at (alpha*, lambda*) and evaluate on OUTER-HOLDOUT
#
## ---- Setup ----
...
### ---- Nested Cross-Validation for PV1MATH (glmnet-only inner) ----
...
tic("Nested Cross-Validation (glmnet-only inner; grouped-style CVM/SE; lambda.min & lambda.1se)")
for (o in seq_len(num_folds_outer)) {
  ...
  w_fold_norm <- tapply(w_train_outer, foldid_inner, sum) / sum(w_train_outer)
  
  # --- Inner tuner over α-grid using glmnet-only manual CV + OOF reconstruction ---
  ...
  tic(sprintf("Inner tuning (glmnet-only) — outer fold %d", o))
  for (i in seq_along(alpha_grid)) {
    alpha <- alpha_grid[i]
    message(sprintf("  [Inner α %d/%d] alpha = %.1f — fit OUTER-TRAIN path + manual CV/OOF ...",
                    i, length(alpha_grid), alpha))
    
    # ---- Fit on full OUTER-TRAIN to set the reference λ grid ----
    fit <- glmnet::glmnet(
    ...
    # ---- Manual CV: train on K-1 folds, predict on held-out fold aligned to the reference λ grid ----
    oof_pred <- matrix(NA_real_, nrow = n_cv_inner, ncol = L)
    fold_mse <- matrix(NA_real_, nrow = num_folds_inner, ncol = L)
    
    for (k in seq_len(num_folds_inner)) {
      idx_valid_k_loc <- inner_folds[[k]]               # positions in OUTER-TRAIN space
      idx_train_k_loc <- setdiff(seq_len(n_cv_inner), idx_valid_k_loc)
      
      fit_k <- glmnet::glmnet(
        x = X_train_outer[idx_train_k_loc, , drop = FALSE],
        y = y_train_outer[idx_train_k_loc],
        weights = w_train_outer[idx_train_k_loc],
        family = "gaussian", alpha = alpha,
        standardize = TRUE, intercept = TRUE
      )
      
      # Align predictions to the reference λ grid (interpolates if needed)
      pred_k <- predict(
        fit_k,
        newx = X_train_outer[idx_valid_k_loc, , drop = FALSE],
        s    = fit$lambda,
        exact = FALSE
      )
      oof_pred[idx_valid_k_loc, ] <- pred_k
      
      wk <- w_train_outer[idx_valid_k_loc]
      yk <- y_train_outer[idx_valid_k_loc]
      fold_mse[k, ] <- colSums(wk * (yk - pred_k)^2) / sum(wk)
    }
    
    # ---- Grouped-style CVM & SE (cv.glmnet(grouped=TRUE)-compatible summaries) ----
    # Fold-weighted mean MSE
    cvm_grouped <- as.numeric(t(fold_mse) %*% w_fold_norm)
    # Weighted sample SD across folds (same scale as cv.glmnet's cvsd; no /sqrt(K))
    cvsd_grouped <- sqrt(
      colSums( sweep(sweep(fold_mse, 2, cvm_grouped, "-")^2, 1, w_fold_norm, "*") ) / (num_folds_inner - 1)
    )
    
    # (Optional sanity) pooled OOF MSE equals fold-weighted mean when using fold shares
    # cvm_oof <- colSums(w_train_outer * (y_train_outer - oof_pred)^2) / sum(w_train_outer)
    # stopifnot(isTRUE(all.equal(cvm_grouped, as.numeric(cvm_oof))))
    
    # ---- Rule-based λ selection for this α (tie-breaks → larger λ) ----
    # lambda.min
    ...
    # lambda.1se (grouped)
    lambda_1se  <- max(fit$lambda[cvm_grouped <= (cvm_grouped[idx_min] + cvsd_grouped[idx_min])])
    idx_1se     <- match(lambda_1se, fit$lambda)
    
    # Save per-α winners for both rules
    per_alpha_list_min[[i]] <- tibble::tibble(
      ...
      lambda      = lambda_min,
      lambda_idx  = idx_min,
      nzero       = fit$df[idx_min],
      dev_ratio   = as.numeric(fit$dev.ratio[idx_min]),
      cvm         = cvm_grouped[idx_min],
      cvsd        = cvsd_grouped[idx_min],
      cvlo        = cvm_grouped[idx_min] - cvsd_grouped[idx_min],
      cvup        = cvm_grouped[idx_min] + cvsd_grouped[idx_min]
    )
    ...
    per_alpha_list_1se[[i]] <- tibble::tibble(
      ...
      lambda      = lambda_1se,
      lambda_idx  = idx_1se,
      nzero       = fit$df[idx_1se],
      dev_ratio   = as.numeric(fit$dev.ratio[idx_1se]),
      cvm         = cvm_grouped[idx_1se],
      cvsd        = cvsd_grouped[idx_1se],
      cvlo        = cvm_grouped[idx_1se] - cvsd_grouped[idx_1se],
      cvup        = cvm_grouped[idx_1se] + cvsd_grouped[idx_1se]
    )
  } # α loop
  toc()
  ...
  # --- Outer refit & scoring at each rule's winner ---
  
  # lambda.min
  fit_outer_min <- glmnet::glmnet(
    x = X_train_outer, y = y_train_outer, weights = w_train_outer,
    family = "gaussian",
    alpha = best_min$alpha, lambda = best_min$lambda,
    standardize = TRUE, intercept = TRUE
  )
  ...
  # lambda.1se
  fit_outer_1se <- glmnet::glmnet(
    x = X_train_outer, y = y_train_outer, weights = w_train_outer,
    family = "gaussian",
    alpha = best_1se$alpha, lambda = best_1se$lambda,
    standardize = TRUE, intercept = TRUE
  )
...
toc()
# > Nested Cross-Validation (glmnet-only inner; grouped-style CVM/SE; lambda.min & lambda.1se): 3.089 sec elapsed

#### ---- Aggregations over outer holds (both rules) ----

# Helper to aggregate & print for a given rule
...
message("\n==== Nested-CV aggregated performance (glmnet-only inner; both rules) ====")
...
message("\n==== Inner winners by outer fold — lambda.min (glmnet-only inner) ====")
...
message("\n==== Inner winners by outer fold — lambda.1se (glmnet-only inner) ====")
...
message("\n==== Consolidated (winners + outer performance) — glmnet-only inner, both rules ====")
